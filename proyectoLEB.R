...
ggsave(glue("tovsefg.png"), p, w = 8, h = 8, dpi = "retina", type = "cairo")

# DER VS OER

p2 <- ggplot(Avanzadas, aes(x = DER, y = OER)) +
  ggplot2::geom_abline(slope = -3 / 4, intercept = seq(11.7, 40.7, 5), alpha = .2) +
  ...
  ggplot2::labs(
    ...
    subtitle = "LEB PLATA Liga Regular ESTE | hasta la J7"
  ) +
  ggplot2::theme_minimal() +
  theme(
    plot.title.position = "plot",
